...
### 문제 발견
- 특정 기준에 충족되는 게시글을 5분마다 DB에서 가져와야하는 상황에서, 여러 서버에서 동일한 인기글을 갱신할수 있도록 보장해줘야하는 문제 발견

### 해결과정
**redis의 분산락을 선택하면서 고려했던점**
- redis 분산락말고 비용이 더 적은 방법이 있는지
  
  1. redis 도입하지 않고 해결해 볼 수 있는지
     
      - 특정 시간대에 모든 서버에서 동시에 실행되기때문에 DB락을 이용해 해결해 볼수 있겠지만 최악의 경우 서버 1과 서버 2가 다른 인기글을 select 할수도 있다는 문제가 있다.
      - 트랜잭션 내에서 유효한 DB락이나 Optimistic lock 은 서버1 select -> 게시글에 update 발생 -> 서버 2, 3 select ... 의 시나리오에서 서로다른 서버가 다른 인기글을 선별할 우려가 있어서 고려하지않았음
      - redis를 도입해 각 서버에서 여러번 select 해서 데이터를 일치시키려는것 보다는, 분산락으로 락이 걸려있다면 재시도 하지않고 fail 하는 방식이 적당하다고 판단하였음
    
  2. redis 도입
      - 락을 걸고 해제할때마다 네트워크 통신비용이 들지만 5분에 한번씩만 실행되는 성질이므로 크게 문제 되지않을것 같다.
      - DB가 락을 보장하는등의 성질이 아닌 개발자가 직접 구현해야하므로
      - 락을 redis로 관리하게 되면서 레디스에 문제가 생긴다면 데이터 손실등의 위험이있지만, 인기글의 조회수의 손실은 크리티컬 하지않기 때문에 천천히 고려해도 될것같다. 
...
### 한계점
- 당장 성능비교를 해보고 hash보다 성능이 좋은방법을 선택했으나, 왜 hash를 사용했을때 성능이 안좋은지 파악하지 못했음. redis 자체에대한 학습이 좀더 필요하거나 필요해진다면 apm등으로 왜 속도가 느린건지 파악해봐야 할수있음. 
