...
300vuser / 1sec / 1loop : 5.5초의 레이턴시 + 커넥션풀에 110개의 요청이 대기(무조건 개선해야하는 문제)
...
- 조회수를 매번 update해서 DB에 바로 반영할게 아니라 캐싱후 카운팅하고 주기적으로 반영하면 되지않을까?
  - 서버가 1대라면 인메모리캐시나 동기화된 자료구조를 사용해 조회수를 카운팅하고 주기적으로 반영해보자
     - 이 방식은 비용을 최소화 할수있지만 조회수 정합성이 바로바로 보장이 안되는 단점이 있지만, update 쿼리 발생으로 인한 성능저하 우려가 있다. 
  - 서버가 N대인 상황을 고려하자. 왜냐하면 국내 커뮤니티 트래픽 조사결과 초당 500~1000개의 이상의 요청이 들어오는경우가 허다했기 때문에 단일서버로 처리하기엔 무리일 확률이 높기때문이다.
     - 조회수 정합성 맞출 필요도 없고, 찾아보니 방문자 통계같은걸 기록하는 정확도를 잃지만 빠른속도(레디스내 연산속도가 O(1)로 고정)와 적은메모리를 사용하는 hyperloglog의 존재를 알게되었고 사용해 볼수있겠다고 생각했다.
...
### 한계점은 무엇이었는지?
- 1300user / 10sec , 350vuser / 1sec 요청의 경우부터 2초이상의 latency가 발생하며, 커넥션 대기가 추가로 발생하였음 
- 조회수의 정확성을 확실하게 맞춰줘야 한다 라는 요구사항이 생긴다면 hyperloglog를 사용할 수 없다.
