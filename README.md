...
1. redis 도입하지 않고 해결해 볼 수 있는지
  ...
  - mysql 에서 제공하는 네임드락으로 해결해보거나, redis를 도입해 분산락을 도입하는 방법 둘중 하나로 해결하는것이 적합하다고 판단하였음
    
2. redis 도입
  - mysql락을 사용한다면 추가적인 인프라 관리비용, redis 유지보수비용을 줄이고 문제를 바로 해결해 볼 수 있겠지만 
...
- 조회수를 매번 update해서 DB에 바로 반영할게 아니라 캐싱후 카운팅하고 주기적으로 반영하면 되지않을까?
  ...
  - 서버가 1대라면 인메모리캐시를 이용해 조회수를 카운팅하고 주기적으로 반영해보자
     - 이 방식은 조회수 정합성이 바로바로 보장이 안되는 단점이 있지만 해당 요구사항엔 감안할수있는 단점이고 비용을 최소화 할수있다.
...
- 현재 redis에서 관리하는건 인기글리스트와 카운팅된 조회수뿐이므로 유실되더라도 당장 서비스하는데에는 문제없기때문에 우선순위가 낮을수있다.
