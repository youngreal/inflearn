## 인프런의 질문/스터디 모집 서비스
- 기능을 먼저 구현하고, 실제로 많은 회원이 사용한다면 어떻게 대응할수 있을지 대응방법을 경험해보기
...
- 서버간 이벤트를 공유하는가?   
  ...
  3. 단지 비용만 아끼고, ``결국에 나중에 누군가 확장해야할 레거시코드가 되진않을지``?
      - 회원가입에 트래픽이 엄청나게 몰리는 선착순 이벤트같은게 당장 예정되어있다면 그럴수도있다. 다만, 서버 scale-out이나, nginx의 로드밸런싱, rate limiter 등을 도입하는 방법도 있기 때문에 무조건 이 코드가 변경되어야한다는 보장은 없다.
