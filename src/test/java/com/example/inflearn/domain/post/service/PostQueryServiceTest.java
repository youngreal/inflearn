...
class PostQueryServiceTest {
    ...
    void 검색어로_게시글을_검색하면_검색어를_포함하고있는_게시글목록을_반환한다() {
        ...
        Pageable pageable = PageRequest.of(0, 10, Sort.by(Sort.Order.desc("createdAt")));
        given(postRepository.findByTitleContainingOrContentsContaining(searchWord, searchWord, pageable)).willReturn(List.of());
        ...
        // then
        assertThat(actual).isEmpty();
        then(postRepository).should().findByTitleContainingOrContentsContaining(searchWord, searchWord, pageable);
    }
}